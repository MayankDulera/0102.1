...
for nested_link in finallink:
    try:
        ...
        datalink = {{"link":nested_link}}
        scraped_data.append(datalink)
    except Exception as e:
        continue
    for xpath_dict in xpath_list:
        ...
        try:
            ...
            if element:
                if element[0].get_attribute('src'):
                    data = {{label: element.get_attribute('src')}}
                    scraped_data.append(data)
                else:
                    finaldata = ""
                    if len(element) != 0:
                        for text_data in element:
                            if text_data.text != "":
                                finaldata = finaldata + text_data.text
...
''' 
    ...
    try:
        ...
        script_file_path = "/home/bisag/Downloads/rapp/backend-main/backend/user_crons"
        script_file_path_store = os.path.join(script_file_path, script_name)
        cursor.execute("INSERT INTO cronscripts (cron,time,domain,script_path) VALUES (%s,%s,%s,%s)", (code_to_save,crontime,domain,script_file_path_store))
        conn.commit()
        create_cron_job(code_to_save, crontime, domain,script_file_path,script_name,script_file_path_store)
...
def create_cron_job(script, crontime, domain,script_file_path,script_name,script_file_path_store):
    ...
    try:
        ...
        log_directory = '/home/bisag/Downloads/rapp/backend-main/backend/user_crons'
        log_file_path = os.path.join(log_directory, 'file.log')
        # logging.basicConfig(filename=log_file_path, level=logging.INFO, format='%(asctime)s - %(levelname)s: %(message)s')
        with open(script_file_path_store, "w") as script_file:
            script_file.write(script)
        command = "/usr/bin/python3 "+script_file_path_store+ " >> " + log_file_path + " 2>&1"

        schedule = str(crontime.split(":")[1]) + " " + str(crontime.split(":")[0]) + " * * *"
        cron = CronTab(user='bisag')
        ...
        print("innn",e)
    # Add the cron job
    # cron_command = f'python {script_file_path}'
    # cron_job = f'{crontime} {cron_command}\n'
